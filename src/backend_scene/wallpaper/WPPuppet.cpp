...
static Quaternionf ToQuaternion(Vector3f euler) {
        ...
        return AngleAxis<float>(euler.z(), axis[2]) *
            AngleAxis<float>(euler.y(), axis[1]) *
...
static constexpr void genInterpolationInfo(WPPuppet::Animation::InterpolationInfo& info, double& cur, uint length, double frame_time, double max_time) {
...
WPPuppet::Animation::InterpolationInfo WPPuppet::Animation::getInterpolationInfo(double* cur_time) const {
    ...
    if(mode == PlayMode::Loop || mode == PlayMode::Single) {
        ...
        const auto _get_frame = [this](auto f) { return f >= length ? (length-1) - (f-length) : f; };
