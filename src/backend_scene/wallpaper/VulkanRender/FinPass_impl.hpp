...
void FinPass::setPresentLayout(vk::ImageLayout layout) { m_desc.present_layout = layout; }
void FinPass::setPresentFormat(vk::Format format) { m_desc.present_format = format; }
...
void FinPass::prepare(Scene& scene, const Device& device, RenderingResources& rr) {
    ...
    {
        bind_description.setStride(sizeof(VertexInput))
...
void FinPass::execute(const Device& device, RenderingResources& rr) {
    ...
    {
        vk::DescriptorImageInfo desc_img { m_desc.vk_result.sampler,
                                           m_desc.vk_result.view,
