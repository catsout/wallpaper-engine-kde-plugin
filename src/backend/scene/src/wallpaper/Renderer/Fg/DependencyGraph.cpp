...
void DependencyGraph::Connect(NodeID n1, NodeID n2) {
	m_nodeNext[n1].insert(n2);
}

typedef std::function<std::vector<NodeID>(NodeID)> NextNodeOp;
typedef std::function<void(NodeID)> DfsCallbackOp;
static void Dfs(NodeID id, std::vector<bool>& rec, const NextNodeOp& next, const DfsCallbackOp& dcOp, const DfsCallbackOp& reverseOp) {
	if(rec[id]) return;
	rec[id] = true;
	if(dcOp) dcOp(id); 
...
std::vector<NodeID> DependencyGraph::TopologicalOrder() const {
	...
	std::vector<NodeID> result;result.reserve(m_nodes.size());
	std::vector<bool> dfsrec(m_nodes.size(), false);
	for(int i=0;i<dfsrec.size();i++) {	
		if(dfsrec[i]) continue;
		Dfs(i, dfsrec, std::bind(&DependencyGraph::GetNodeOut, this, _1), DfsCallbackOp(), [&](NodeID i) {
			result.push_back(i);
		});
	}
...
node [shape=box]
	...
	for(int i=0;i<m_nodeNext.size();i++) {
